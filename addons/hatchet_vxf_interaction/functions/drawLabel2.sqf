...
with uiNamespace do {
  if (isNil "vtx_cursor_ctrl") then {
    vtx_cursor_ctrl = findDisplay 46 ctrlCreate ["RscStructuredText", -1];
  };
};
...
if (isNil "vxf_interaction_currentButton") exitWith {
  ...
  //["",0,0.4,0,0,0, 1301] spawn BIS_fnc_dynamicText;
  ["", ""] call vxf_interaction_fnc_drawLabel2_set;
};
...
//    ["Scroll or drag to spin",0,0.6,0,0,0, 1303] spawn BIS_fnc_dynamicText;
    ["", "Scroll or drag to spin"] call vxf_interaction_fnc_drawLabel2_set;
  } else {
...
if (count _animConfig > 0 && count _knobConfig == 0) then {
  ...
  if (vxf_interaction_showLabels > 1) then {
    ...
    [_prevText, _nextText] call vxf_interaction_fnc_drawLabel2_set;
  } else {
    //["",0,0.4,0,0,0, 1301] spawn BIS_fnc_dynamicText;
