...
with uiNamespace do {
  if (isNil "vtx_cursor_ctrl") then {
    vtx_cursor_ctrl = findDisplay ([46, 86005] select vxf_interaction_mouseBlocker) ctrlCreate ["RscStructuredText", -1];
  };
};
uiNamespace setVariable ["vxf_interaction_cursorPos", vxf_interaction_cursorPos];

if (isNil "vxf_interaction_currentButton") exitWith {
  ["", ""] call vxf_interaction_fnc_setLabel;
};
...
//    ["Scroll or drag to spin",0,0.6,0,0,0, 1303] spawn BIS_fnc_dynamicText;
    ["", "Scroll or drag to spin"] call vxf_interaction_fnc_setLabel;
  } else {
    ["", ""] call vxf_interaction_fnc_setLabel;
  };
} else {
...
if (count _animConfig > 0 && count _knobConfig == 0) then {
  ...
  if (vxf_interaction_showLabels > 1) then {
    ...
    [_prevText, _nextText] call vxf_interaction_fnc_setLabel;
  } else {
    ["", ""] call vxf_interaction_fnc_setLabel;
  };
