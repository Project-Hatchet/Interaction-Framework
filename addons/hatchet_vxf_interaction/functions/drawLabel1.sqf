...
/*
 * vxf_interaction_fnc_drawLabel Execution Time: 0.0146 ms
 *
 * interaction button release functionality
 */
...
//    ["Scroll or drag to spin",0,0.6,0,0,0, 1303] spawn BIS_fnc_dynamicText;
    drawIcon3D [
      "",
      [1,1,1,1],
      positionCameraToWorld [0,0,0] vectorAdd (screenToWorldDirection (vxf_interaction_cursorPos vectorAdd [0,0.1])),
      1,1,0,
  ...
  if (vxf_interaction_showLabels > 1) then {
    if (_prevStep > -1) then {
      ...
      drawIcon3D [
        ...
        positionCameraToWorld [0,0,0] vectorAdd (screenToWorldDirection (vxf_interaction_cursorPos vectorAdd [0,-0.1])),
        1,1,0,
        //format ["<t size='0.8'>Set to <t color='#ff0000'>%2</t> with <t color='#ff0000'>%1</t></t>", vxf_akn_prev, _animationLabels # _prevStep]
        format ["Set to %2 with %1", vxf_akn_prev, _animationLabels # _prevStep]
      ];
    } else {
      //["",0,0.4,0,0,0, 1301] spawn BIS_fnc_dynamicText;
    };
    if (_nextStep > -1) then {
      ...
      drawIcon3D [
        ...
        positionCameraToWorld [0,0,0] vectorAdd (screenToWorldDirection (vxf_interaction_cursorPos vectorAdd [0,0.1])),
        1,1,0,
        //format ["<t size='0.8'>Set to <t color='#ff0000'>%2</t> with <t color='#ff0000'>%1</t></t>", vxf_akn_next, _animationLabels # _nextStep]
        format ["Set to %2 with %1", vxf_akn_next, _animationLabels # _nextStep]
      ];
    } else {
