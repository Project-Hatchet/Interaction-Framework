 ...
 * the most relevant one will be assigned to the vehicle's vxf_core_config variable
 *
...
private _turretIndex = [vxf_player] call EFUNC(util,getTurretIndex);

if (vxf_player == driver _vehicle) then {
...
if (vxf_player == gunner _vehicle) then {
  _configSources pushBack QRVAR(gunner);
};

if (_vehicle getCargoIndex vxf_player > -1) then {
