...
private _color = if (hct_interaction_buttonHolding) then {[1,0,0,1]} else {[1,1,1,1]};
private _size = if (hct_interaction_buttonHolding) then {0.045} else {0.05};
_label = if (hct_interaction_showLabels > 0.5) then {
  if (count _buttonConfig > 0 && count _knobConfig == 0) then {
    private _fmt = if (hct_interaction_showLabels > 1) then {
      "[%1] %2"
    } else {
      "%2"
    };
    format [_fmt, hct_akn_use, _label]
  } else {
    _label
  }
} else {
