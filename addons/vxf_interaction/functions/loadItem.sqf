...
{ //forEach _subItems
  _subItems pushBack ([_vehicle, _x] call vxf_interaction_fnc_loadItem);
...
if (_animation != "" && _dragRange > 0 && ISFULLARRAY(_animLimits)) then {
  ...
  _buttonConfig = [
    {_this call vxf_interaction_fnc_dragStart},
    {_this call vxf_interaction_fnc_dragStop},
    {_this call vxf_interaction_fnc_drag},
    compile _buttonUp
  ];
};
